...
router.get('/', (req, res) => {
  ...
  if (q) {
    condizioni.push('(nome LIKE ? OR cliente LIKE ? OR citta LIKE ? OR luogo LIKE ?)');
    const like = `%${q}%`;
...
router.post('/', (req, res) => {
  ...
  const info = db.prepare(`
    ...
    VALUES (@nome, @cliente, @luogo, @citta, @padiglione, @stand, @data_inizio, @data_fine,
            @giorni_allestimento, @giorni_smontaggio, @stato, @note, @creato_il, @aggiornato_il)`)
...
router.put('/:id', (req, res) => {
  ...
  const dati = leggiCorpo(req.body);
  db.prepare(`
    UPDATE fiere SET nome=@nome, cliente=@cliente, luogo=@luogo, citta=@citta,
